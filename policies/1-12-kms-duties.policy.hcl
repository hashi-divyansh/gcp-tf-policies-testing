...
# entire project IAM policy in one shot, so the Admin role and a
# conflicting crypto role can appear in the very same policy_data
# document. Its bindings must be decoded and checked both against each
# other and against any google_project_iam_binding/member resources
# declared for the same project.
